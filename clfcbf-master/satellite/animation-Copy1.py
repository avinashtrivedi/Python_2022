...
def animate(i):
    ...
    line = ax.plot(ss[i*10][0, 0].item(),ss[i*10][0, 1].item(),marker='o', color='blue')
    return line

ani = FuncAnimation(fig, animate, interval=300, blit=True, repeat=False,save_count=0 ,frames=len(ss[::10]))    
ani.save("TLI11.gif", dpi=300, writer=PillowWriter(fps=3))
