...
def map_name_growth_cell_(path, exceptions = os.path.join(sai_dir, "P_MD_Constants\exceptions.csv")):
    ...
    gap_dict = {'Health - Health Analytics': 'Health - Payer Transformation - Payer Analytics',
    'Health - Payer CO&I' : 'Health - Payer Transformation - CO&I',
    'Health - Payer Clinical Transformation': 'Health - Payer Transformation - Clinical Transformation',
    'Health - Payer Government Programs' : 'Health - Payer Transformation - Government Programs',
    'Health - Provider ERP' : 'Health - Provider Transformation - ERP',
    'Health - Provider FFG' : 'Health - Provider Transformation - Fit For Growth',
    'Health - Provider Revenue Cycle' : 'Health - Provider Transformation - Revenue Cycle',
    'Health - R&D Future of Clinical Trials' : 'Health - Clinical R&D - Future of Clinical Trials'}
    wd_info['Growth Cell'] = wd_info['Growth Cell'].apply(lambda x: gap_dict[x] if x in gap_dict.keys() else x)
...
def map_name_growth_cell(path = sai_dir + 'WoW Hours - All Staff.csv'):
...
def salesforce_pipe_agg(path_sf = os.path.join(sai_dir,"Daily Transformation Pipe Exports"), path_wtd_sf = os.path.join(sai_dir,"Consulting Solutions Pipe Exports")):

    pipe_sf = salesforce_pipeline(path_sf)
    pipe_sf_wtd = salesforce_pipeline(path_wtd_sf)
    merged = pipe_sf.merge(pipe_sf_wtd[['Opportunity ID', 'Opportunity Weighted Amount']], left_on='Opportunity ID', right_on="Opportunity ID")

    pipe_sf_agg = merged[merged['Created Date']> datetime.datetime(2021, 7, 1)].groupby(['Growth Cell', 'Created_date_year', 'Created_date_month']).agg({'Amount (converted)': 'sum', 'Opportunity Weighted Amount':'sum' , 'Opportunity ID':'count', 'Ultimate Parent Account Name':pd.Series.nunique}).reset_index()
...
def hours_charged(path_to_folder = os.path.join(sai_dir,'Hours Charged')):

    mapper = map_empid_growth_cell()
    all_dfs_hours = {}
    for file in os.listdir(path=path_to_folder):
        if 'csv' in file:
            all_dfs_hours[file] = pd.read_csv(os.path.join(path_to_folder, file), encoding='latin1')
    
    all_dfs = []
    for file, hours in all_dfs_hours.items():
        hours = hours[~ hours['Workday ID'].isna()]
        hours[['Client',
               '4 Code', 'Holiday', 'Military Leave', 'Leave',
               'HR / Recruiting', 'Idle', 'Marketing', 'Other', 'PD Code', 'Sick',
               'Training', 'Vacation', 'Capacity']] =  hours[['Client',
               '4 Code', 'Holiday', 'Military Leave', 'Leave',
               'HR / Recruiting', 'Idle', 'Marketing', 'Other', 'PD Code', 'Sick',
               'Training', 'Vacation', 'Capacity']].astype(float)

        hours['Growth Cell'] = hours['Workday ID'].apply(lambda x: mapper[x] if x in mapper.keys() else 'Cannot Find')
        hours['Levels'] = hours['Level'].apply(lambda x: 'Manager-Director' if x in ['Manager', 'Senior Manager', 'Director'] else ('D+' if x in ['Managing Director', 'Partner'] else 'M-'))
        hours = hours.groupby(['Growth Cell', 'Levels'])[['Client',
               '4 Code', 'Holiday', 'Military Leave', 'Leave',
               'HR / Recruiting', 'Idle', 'Marketing', 'Other', 'PD Code', 'Sick',
               'Training', 'Vacation','Capacity']].agg('sum').reset_index()
        hours['Frac Client Code'] = hours['Client']/hours['Capacity']
        hours['Frac 4 Code'] = hours['4 Code']/hours['Capacity']
        hours['Frac PD Code'] = hours['PD Code']/hours['Capacity']
        hours = hours[['Growth Cell', 'Frac Client Code', 'Frac 4 Code', 'Frac PD Code', 'Levels']]
        k = hours.pivot(index='Growth Cell', columns= 'Levels', values=['Frac Client Code', 'Frac 4 Code', 'Frac PD Code', 'Growth Cell']).reset_index(drop = True)
        df = pd.DataFrame()
        df['Frac 4 Code D+'] = k['Frac 4 Code']['D+']
        df['Frac 4 Code M-'] = k['Frac 4 Code']['M-']
        df['Frac 4 Code Manager-Director'] = k['Frac 4 Code']['Manager-Director']
        df['Frac PD Code D+'] = k['Frac PD Code']['D+']
        df['Frac PD Code M-'] = k['Frac PD Code']['M-']
        df['Frac PD Code Manager-Director'] = k['Frac PD Code']['Manager-Director']
        df['Frac Client Code D+'] = k['Frac Client Code']['D+']
        df['Frac Client Code M-'] = k['Frac Client Code']['M-']
        df['Frac Client Code Manager-Director'] = k['Frac Client Code']['Manager-Director']
        df['Growth Cell'] = k['Growth Cell']['D+']
        df['Year'] = int(file.split('_')[1].split('.')[0])
        df['Month'] = int(file.split('_')[0])
        df = df[~df['Growth Cell'].isna()]
        all_dfs.append(df)
...
def utilization_trends(path = os.path.join(sai_dir,'07.01.22.xlsx - Export.csv')):
    ...
    utilizn_growth_cell_mean_dev = utilizn_growth_cell_util_diff.groupby(['Growth Cell', 'Level'])[['21-Jul', '21-Aug', '21-Sep', '21-Oct', '21-Nov', '21-Dec', '22-Jan',
       '22-Feb', '22-Mar', '22-Apr', '22-May', '22-Jun']].agg({'21-Jul': 'mean',
                                                                       '21-Aug': 'mean',
                                                                       '21-Sep': 'mean',
                                                                       '21-Oct': 'mean',
                                                                       '21-Nov': 'mean',
    ...
    utilizn_growth_cell_mean_dev = transpose_df(utilizn_growth_cell_mean_dev, '_Mean_Dev')
...
def create_all_data(all_data):
    ...
    all_data_df = all_data_df.drop(columns = ['Year_x', 'Month_x', 'Year_y', 'Month_y','Growth Cell_y', 'Growth Cell_x'])
